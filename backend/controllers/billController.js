...
const createBill = async (req, res) => {
  try {
    ...
    const populatedBill = await Bill.findById(bill._id).populate('createdBy', 'fullName username');
...
const getBillById = async (req, res) => {
  try {
    const bill = await Bill.findById(req.params.id).populate('createdBy', 'fullName username');
...
const generateBillPDF = async (req, res) => {
  try {
    const bill = await Bill.findById(req.params.id).populate('createdBy', 'fullName');
    ...
    doc.moveDown();
    doc.text(`Generated by: ${bill.createdBy.fullName}`, { align: 'center' });
